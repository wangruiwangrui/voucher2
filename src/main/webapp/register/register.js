...
//---------------------------username-----------------------------------------------------	
  ...
  $('#username input').blur(function(){
     var reque=$('#username input')[0].value;
   	$.post('/voucher/register/testName.do',{
   	   username:reque
   	   },function(text){ 
...
//-------------------------------密码强度测试模块结束-------------------------------------------------
   ...
   $('#password input').blur(function(){
	   ...
	   $.post('/voucher/register/testPassword.do',{
		  password:reque
		 },function(text){
			var obj=JSON.parse(text);
			data=obj.data;
...
//--------------------------------------------------------------------------------
     ...
     $('#telephone input').blur(function(){
     	...
     	$.post('/voucher/register/testPhone.do',{
     		telephone:reque
     		},function(text){
     		var obj=JSON.parse(text);
     		 data=obj.data;
...
//---------------------------------email--------------------------------------------------  
  ...
  $('#email input').blur(function(){
	 var reque=$('#email input')[0].value;
   	...
   	$.post('/voucher/register/testEmail.do',{
   	   email:reque
   	  },function(text){  
   	    var obj=JSON.parse(text);
		data=obj.data;
   ...
   if(valid1==true&&valid2==true&&valid3==true&&valid4==true){
	 ...
	 if(!chk.checked){                                   //没有同意协议复选框选择 
	   ...
	   var textver2='<span id="moblie_ver"><span class="wrong"><div class="action_po_top"><p>验证码错误</p></div></span></span>';
	    $.post('/voucher/register/insert.do', 
