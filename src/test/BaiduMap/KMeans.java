...
public class KMeans {
    ...
    public Map doKMeans(){
        ...
        Map map=new HashMap<>();
        map.put("centerPoints", centerPoints);
    ...
    public double calDistance(Point point1, Point point2){
        double d=Math.pow((point1.getX() - point2.getX()), 2) + Math.pow((point1.getY() - point2.getY()), 2);
    ...
    public void show(List<Point> centerPoints, HashMap<Integer, List<Point>> clusters){
        for (int i=0; i<centerPoints.size(); i++){
            System.out.print("类{0}的中心点: <{1}, {2}> "+(i+1)+": "+centerPoints.get(i).getX()+","+centerPoints.get(i).getY());
            List<Point> lists = clusters.get(i);
